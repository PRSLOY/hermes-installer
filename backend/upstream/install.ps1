...
# Subscriber patch: every large asset (PortableGit ~57MB, Node ~30MB, repo ZIP) is fetched
# from github.com/nodejs.org with a single bare Invoke-WebRequest. A CIS consumer link
# measured ~520 KB/s with frequent resets, so one dropped connection ended the whole install
# -- observed live: the git stage died with exit 1 (git-for-windows download). Retry with
# backoff and resume where the server supports it.
function Invoke-DownloadWithRetry {
    ...
    for ($attempt = 1; $attempt -le $Attempts; $attempt++) {
        try {
            ...
            # Only the exception message type is surfaced; never the raw body.
            Write-Warn ("  {0} attempt {1}/{2} failed: {3}" -f $Label, $attempt, $Attempts, $_.Exception.Message)
        }
...
# Subscriber patch: download from the first source that yields a complete file.
# Used to reach GitHub-hosted assets when github.com itself is blocked/throttled
# (common on Russian consumer links): binary mirrors (npmmirror/huaweicloud) for
# PortableGit, and GitHub archive proxies for the repository ZIP.
...
# Subscriber patch: non-GitHub routes for networks that block github.com.
# Verified reachable and byte-identical from a Russian host (2026-09-21):
#   npmmirror/huaweicloud serve PortableGit (Content-Length 58995352);
#   ghproxy.net / gh-proxy.com proxy the repository archive (HTTP 200).
# Each is tried only after the direct GitHub URL fails, so reachable networks
# keep the fast path.
$script:GitHubZipProxies = @(
    'https://ghproxy.net/',
    'https://gh-proxy.com/'
)
$script:GitForWindowsMirrors = @(
...
function Install-Git {
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    try {
        ...
        # Direct GitHub first, then binary mirrors for blocked-github networks.
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if ($Commit) {
                    ...
                    # Subscriber patch: the managed clone is shallow, so an
                    # unbounded fetch of an older commit tries to pull the whole
                    # history and dies with a transfer error. Bounded fetch only.
                    # Retry: the transfer is a single long HTTPS body and blips.
                    $fetchOk = $false
                    for ($attempt = 1; $attempt -le 3; $attempt++) {
                        git -c windows.appendAtomically=false fetch --depth=1 origin $Commit
                        if ($LASTEXITCODE -eq 0) { $fetchOk = $true; break }
                        if ($attempt -lt 3) {
                            Write-Warn "Fetching the pinned commit failed (exit $LASTEXITCODE); retrying ($attempt/3)..."
                            Start-Sleep -Seconds (2 * $attempt)
                        }
                    }
                    if (-not $fetchOk) {
                        throw "git fetch $Commit failed (exit $LASTEXITCODE)"
                    }
    ...
    if (-not $didUpdate) {
        ...
        # Also pin core.autocrlf=false AT CLONE TIME: Git for Windows defaults to
        # autocrlf=true and checks the tree out as CRLF. Setting the config later
        # (below) makes that CRLF tree look dirty, and the pinned `git checkout`
        # then aborts with "Your local changes would be overwritten by checkout"
        # -- the fresh-install repository failure. Cloning with the pin already
        # active means the working tree is LF and stays clean.
    ...
    if (-not $didUpdate) {
        ...
        try {
            if ($Commit) {
                ...
                # so retry a few times before giving up -- a single blip must not kill the install.
                $fetchOk = $false
                for ($attempt = 1; $attempt -le 3; $attempt++) {
                    git -c windows.appendAtomically=false fetch --depth=1 origin $Commit
                    if ($LASTEXITCODE -eq 0) { $fetchOk = $true; break }
                ...
                if (-not $fetchOk) {
                    throw "git fetch pinned commit failed (exit $LASTEXITCODE)"
                }
                # -f: the pinned checkout must not be blocked by working-tree churn
                # (autocrlf/line-ending renormalization) on a freshly cloned tree.
...
# Electron payload fallback sources, in order (GitHub ~520 KB/s from a CIS consumer link; a
# ~150 MB payload plus a 15-minute desktop stage means speed decides whether the install
# finishes). Measured from INSIDE Windows Sandbox on 2026-09-18, electron 40.10.2, real bytes:
#   cdn.npmmirror.com/binaries/electron/  CustomDir="{{ version }}" -> HTTP 206, 2866 KB/s
#   mirrors.huaweicloud.com/electron/     CustomDir="{{ version }}" -> HTTP 206, 1667 KB/s
# Each entry carries its own CustomDir because @electron/get builds
#   <Mirror><CustomDir>/<filename>
# and the default CustomDir ("v"+version) 404s/timeouts on these mirrors -- the version must be
# used bare. The previous single default (npmmirror.com/mirrors/electron/) was measured
# unreachable from the sandbox, so the retry it powered failed too.
# ORDER MATTERS: the first entry is what the pre-seed and `npm ci` use. huaweicloud is first
# because it was the one VERIFIED from inside Windows Sandbox (2026-09-18): a fresh
# `npm install electron@40.10.2` returned exit 0 with electron.exe present in 33 SECONDS,
# while npmmirror stalled there (>7min with no bytes). Never reorder without re-measuring.
$script:DesktopElectronFallbackMirrors = @(
    @{ Mirror = "https://mirrors.huaweicloud.com/electron/";    CustomDir = "{{ version }}" },
    @{ Mirror = "https://cdn.npmmirror.com/binaries/electron/"; CustomDir = "{{ version }}" }
)
...
function Approve-ElectronInstallScript {
    ...
    # opts in, and it still exits 0:
    #   npm warn install-scripts 1 package had install scripts blocked because they are not
    #     covered by allowScripts: electron@40.10.2 (postinstall: node install.js)
    # electron's payload (~138MB) is downloaded BY that postinstall, so on npm 12 the tree
    # ends up without electron.exe and the failure only surfaces later, from the packer.
    #
    # Verified on the host (npm 12.0.2, fast link):
    #   .npmrc with `allow-scripts[]=electron`       -> postinstall ran, electron.exe 213,947,904 B, 5s
    #   package.json allowScripts                     -> also works
    #   npm_config_allow_scripts env var              -> rejected in project-scoped installs
    #   (no opt-in at all)                            -> exit 0, no dist/, misleading later failure
    # `.npmrc` is the least invasive form: it does not rewrite the upstream manifest, and the
    # file is created only if absent so a user's own .npmrc is never clobbered.
    param([string]$NpmExe, [string]$InstallDir)
    if (-not $NpmExe) { return $false }
    if (-not (Test-Path -LiteralPath $InstallDir)) { return $false }

    # CRITICAL: npm reads .npmrc from the PROJECT ROOT -- the directory the install runs in --
    # not from the workspace subdirectory holding the dependency. `npm ci` below runs in
    # $InstallDir, so a .npmrc placed in apps\desktop is never consulted.
    # Verified in a Windows Sandbox (2026-09-18): same .npmrc contents in the directory where
    # npm actually runs -> postinstall executed, electron.exe present, exit 0, 33s.
    $npmrc = Join-Path $InstallDir '.npmrc'
    $prevEAP = $ErrorActionPreference
    $ErrorActionPreference = 'Continue'
    try {
        $existing = ''
        if (Test-Path -LiteralPath $npmrc) { $existing = Get-Content -LiteralPath $npmrc -Raw }
        if ($existing -notmatch 'allow-scripts') {
            $line = 'allow-scripts[]=electron'
            $sep = if ($existing -and -not $existing.EndsWith("`n")) { "`r`n" } else { '' }
            Set-Content -LiteralPath $npmrc -Value ($existing + $sep + $line + "`r`n") -Encoding ASCII -NoNewline
        }
        Write-Info "  Allowed electron's install script (npm >= 12 blocks these by default)."
        return $true
    } catch {
        return $false
    } finally {
...
function Set-ElectronMirrorEnv {
    # Returns the previous values so the caller can restore them in `finally` -- leaking
    # ELECTRON_MIRROR into later stages would change unrelated behaviour.
    param([hashtable]$Candidate)
    $prev = @{ Mirror = $env:ELECTRON_MIRROR; CustomDir = $env:ELECTRON_CUSTOM_DIR }
    if (-not $Candidate) {
        $env:ELECTRON_MIRROR = $prev.Mirror
        $env:ELECTRON_CUSTOM_DIR = $prev.CustomDir
        return $prev
    }
    $env:ELECTRON_MIRROR = $Candidate.Mirror
    if ($Candidate.CustomDir) { $env:ELECTRON_CUSTOM_DIR = $Candidate.CustomDir }
    return $prev
}

function Restore-ElectronMirrorEnv {
...
# Subscriber patch: pre-seed the Electron payload so the `npm ci` postinstall becomes a cache hit.
# WHY: @electron/get runs INSIDE npm ci's postinstall
#   (timer build:run:postinstall:apps/desktop/node_modules/electron -> `node install.js`),
# and on a github-blocked link that is what kills the whole desktop stage with a bare
#   error RequestError: connect ETIMEDOUT 140.82.121.3:443
# Retrying only the outer npm command re-runs the same postinstall, so it kept failing.
# Verified path (Windows Sandbox, 2026-09-18): a fresh `npm install electron@40.10.2` with
# ELECTRON_MIRROR=https://cdn.npmmirror.com/binaries/electron/ + ELECTRON_CUSTOM_DIR={{ version }}
# completed in 15 SECONDS and produced node_modules/electron/dist/electron.exe. Seeding that
# payload into the shared cache first makes the later postinstall a local cache hit, so the
# github ETIMEDOUT stops being able to fail the install.
function Initialize-ElectronPayloadCache {
    param([string]$NpmExe, [string]$InstallDir, [string]$CacheDir)
    if (-not $NpmExe -or -not $InstallDir -or -not $CacheDir) { return $false }
    $desktopPkg = Join-Path $InstallDir 'apps\desktop\package.json'
    if (-not (Test-Path -LiteralPath $desktopPkg)) { return $false }

    # Use the declared spec (e.g. "^40.10.2") so a version bump upstream is picked up automatically.
    $spec = $null
    try {
        $json = Get-Content -LiteralPath $desktopPkg -Raw | ConvertFrom-Json
        foreach ($sec in @('devDependencies', 'dependencies')) {
            $props = $json.$sec
            if ($props -and $props.electron) { $spec = [string]$props.electron; break }
        }
    } catch { $spec = $null }
    if (-not $spec) { return $false }

    New-Item -ItemType Directory -Force -Path $CacheDir | Out-Null
    $seed = Join-Path $env:TEMP ('hermes-electron-seed-' + [Guid]::NewGuid().ToString('N').Substring(0, 8))
    New-Item -ItemType Directory -Force -Path $seed | Out-Null
    # npm >= 12 blocks dependency install scripts unless the project opts in, and exits 0
    # anyway; electron's payload is fetched BY its postinstall, so without `allowScripts`
    # the seed silently produces nothing. (Verified on the host: allowScripts ->
    # electron.exe 213,947,904 B; without it -> no dist/ at all.)
    Set-Content -LiteralPath (Join-Path $seed 'package.json') `
        -Value '{"name":"hermes-electron-seed","version":"1.0.0","private":true}' -Encoding UTF8
    # npm >= 12 blocks the postinstall that fetches the payload, and exits 0 anyway.
    Set-Content -LiteralPath (Join-Path $seed '.npmrc') -Value 'allow-scripts[]=electron' -Encoding ASCII

    # npm writes progress/warnings to stderr, which would abort the block under 'Stop'.
    $prevEAP = $ErrorActionPreference
    $ErrorActionPreference = 'Continue'
    $prevCache = $env:electron_config_cache
    $env:electron_config_cache = $CacheDir
    $seeded = $false
    # Try EVERY mirror, not just the first: a mirror can answer fine in one network and stall in
    # another (measured: npmmirror stalled from inside Sandbox while huaweicloud served it).
    foreach ($cand in $script:DesktopElectronFallbackMirrors) {
        $prevMirror = Set-ElectronMirrorEnv -Candidate $cand
        Push-Location $seed
        try {
            Write-Info "  Pre-seeding the Electron payload via $($cand.Mirror) ..."
            & $NpmExe install ("electron@" + $spec) --no-audit --no-fund --no-package-lock --include=optional 2>&1 |
                ForEach-Object { "$_" } | Tee-Object -Variable seedOut
            $code = $LASTEXITCODE
            $seeded = ($code -eq 0) -and (Test-Path -LiteralPath (Join-Path $seed 'node_modules\electron\dist\electron.exe'))
        } catch {
            $seeded = $false
        } finally {
            Pop-Location
            Restore-ElectronMirrorEnv -Prev $prevMirror
            $ErrorActionPreference = $prevEAP
            if (-not $seeded) { Remove-Item -LiteralPath (Join-Path $seed 'node_modules') -Recurse -Force -ErrorAction SilentlyContinue }
        }
        if ($seeded) { break }
    }
    Remove-Item -LiteralPath $seed -Recurse -Force -ErrorAction SilentlyContinue
    $env:electron_config_cache = $prevCache
    return $seeded
}
...
function Restore-ElectronDist {
    param([string]$InstallDir, [string]$Mirror, [string]$CustomDir)
    ...
    $prevMirror = Set-ElectronMirrorEnv -Candidate $(if ($Mirror) { @{ Mirror = $Mirror; CustomDir = $CustomDir } } else { $null })
    try {
...
function Try-RestoreElectronDist {
    ...
    foreach ($cand in $script:DesktopElectronFallbackMirrors) {
        if (Restore-ElectronDist -InstallDir $InstallDir -Mirror $cand.Mirror -CustomDir $cand.CustomDir) { return $true }
    }
    return $false
}
...
function Install-Desktop {
    ...
    try {
        ...
        # Subscriber patch: prefer the mirror for the VERY FIRST attempt. The Electron payload
        # is pulled by a postinstall inside `npm ci` (timer
        # build:run:postinstall:apps/desktop/node_modules/electron), so a mirror that only wrapped
        # the retry never helped the attempt that actually mattered. Live evidence (2026-09-18):
        # three consecutive installs died with `AggregateError [ETIMEDOUT]` against
        # 140.82.121.3:443 (github.com) while the mirror answered in ~2s at ~2.8 MB/s.
        # GitHub stays as the final fallback below, so a mirror outage cannot block an install.
        # Subscriber patch: seed the payload BEFORE npm ci so its postinstall is a cache hit.
        # The STANDARD @electron/get cache root. Two reasons it must not live under the Hermes
# home: (1) a foreign directory there makes the checkpoint refuse the install, (2) the
# postinstall inside `npm ci` resolves this root by default, so seeding it needs no env.
$electronCacheDir = Join-Path $env:LOCALAPPDATA 'electron\Cache'
        $prevElectronCache = $env:electron_config_cache
        $env:electron_config_cache = $electronCacheDir
        if (Initialize-ElectronPayloadCache -NpmExe $npmExe -InstallDir $InstallDir -CacheDir $electronCacheDir) {
            Write-Info "  Electron payload pre-seeded into the shared cache"
        } else {
            Write-Warn "  Could not pre-seed the Electron payload; the install may still need GitHub"
        }
        $prevInstallMirror = Set-ElectronMirrorEnv -Candidate $script:DesktopElectronFallbackMirrors[0]
        # Subscriber patch: npm >= 12 refuses to run package install scripts unless they are
        # approved, and reports SUCCESS anyway:
        #   npm warn install-scripts 1 package had install scripts blocked ...
        #     electron@40.10.2 (postinstall: node install.js)
        # electron's payload is fetched BY that postinstall, so npm 12 produces a tree with
        # no electron.exe and the failure only surfaces later, from the packer.
        Approve-ElectronInstallScript -NpmExe $npmExe -InstallDir $InstallDir
        & $npmExe ci --include=optional --foreground-scripts 2>&1 | ForEach-Object { "$_" } | Tee-Object -Variable npmOut
        $code = $LASTEXITCODE
        if ($code -ne 0) {
            Write-Info "  npm ci failed (exit $code) -- retrying with npm install..."
            & $npmExe install --include=optional --foreground-scripts 2>&1 | ForEach-Object { "$_" } | Tee-Object -Variable npmOut
            $code = $LASTEXITCODE
        }
        # Subscriber patch: Electron's ~150MB payload is fetched by a postinstall INSIDE
        # `npm ci`/`npm install` (timer build:run:postinstall:apps/desktop/node_modules/electron),
        # NOT by `npm run pack`. The Electron mirror fallback below only wraps the pack step, so a
        # GitHub-blocked link killed the dependency install itself. Measured live in Windows
        # Sandbox (2026-09-18):
        #   error RequestError: connect ETIMEDOUT 140.82.121.3:443
        #   verbose exit 1
        # after which electron dist=False and the desktop stage died much later with
        # "get-windows is not installed" -- far from the real cause. Retry the install once
        # against the mirror so the payload has a second route.
        if ($code -ne 0) {
            Write-Warn "Desktop dependency install failed -- a blocked Electron download is the usual cause."
            # Try the remaining mirrors, then finally plain GitHub (env restored).
            foreach ($cand in $script:DesktopElectronFallbackMirrors) {
                if ($cand.Mirror -eq $prevInstallMirror.Mirror) { continue }
                Write-Warn "Retrying the install against the Electron mirror ($($cand.Mirror))..."
                $prevTry = Set-ElectronMirrorEnv -Candidate $cand
                try {
                    & $npmExe ci --include=optional --foreground-scripts 2>&1 | ForEach-Object { "$_" } | Tee-Object -Variable npmOut
                    $code = $LASTEXITCODE
                    if ($code -ne 0) {
                    ...
                    Restore-ElectronMirrorEnv -Prev $prevTry
                }
                if ($code -eq 0) { break }
            }
        }
        if ($code -ne 0) {
            # Last resort: no mirror env at all, straight to the configured default (GitHub).
            Write-Warn "Mirrors exhausted -- retrying the install without a mirror."
            Restore-ElectronMirrorEnv -Prev $prevInstallMirror
            & $npmExe ci --include=optional --foreground-scripts 2>&1 | ForEach-Object { "$_" } | Tee-Object -Variable npmOut
            $code = $LASTEXITCODE
            if ($code -ne 0) {
                & $npmExe install --include=optional --foreground-scripts 2>&1 | ForEach-Object { "$_" } | Tee-Object -Variable npmOut
        ...
        # Subscriber patch: verify the get-windows optional native payload actually
        # landed. npm treats a failed *optional* dependency as non-fatal: when its
        # node-pre-gyp download breaks (ECONNRESET mid-download on a slow link) npm
        # drops node_modules/get-windows and still exits 0, so the desktop build
        # only dies much later inside stage-native-deps.mjs with a generic
        # "get-windows is not installed" -- far from the real cause. Repair it here
        # while the failure is still attributable, then re-verify.
        # Only the package name and a numeric exit code may enter the log; never
        # raw child output, which can carry credentials or private paths.
        # Run the probe whenever the desktop tree is usable. After an Electron self-heal
        # $code is still non-zero even though the tree was repaired, and gating this on
        # `$code -eq 0` skipped the repair entirely -- measured live 2026-09-18, where the
        # build then died later inside stage-native-deps with "get-windows is not installed".
        $desktopTreeUsable = Test-Path -LiteralPath (Join-Path $InstallDir 'node_modules')
        if ($code -eq 0 -or $desktopTreeUsable) {
            # Probe both hoisted (root) and workspace-local locations: stage-native-deps
            # resolves get-windows from apps/desktop, which sees either.
            $gwRoots = @(
                (Join-Path $InstallDir 'node_modules\get-windows'),
                (Join-Path $InstallDir 'apps\desktop\node_modules\get-windows')
            )
            $gwProbe = {
                foreach ($root in $gwRoots) {
                    $binding = Join-Path $root 'lib\binding'
                    if ((Test-Path -LiteralPath $binding) -and
                        (@(Get-ChildItem -LiteralPath $binding -Recurse -Filter '*.node' -File -ErrorAction SilentlyContinue).Count -gt 0)) {
                        return $true
                    }
                }
                return $false
            }
            $gwHasBinding = & $gwProbe
            if (-not $gwHasBinding) {
                Write-Warn "Optional native dependency get-windows is incomplete (npm reports success for failed optional dependencies). Repairing it..."
                # Two npm paths that LOOK obvious both fail here, verified live in the sandbox:
                #   `npm install <pkg>`          in the workspace root -> E404/ENOWORKSPACES
                #   `npm install <pkg> --workspace apps/desktop` or `--prefix <dir>`
                #                                -> "Cannot read properties of null (reading 'edgesOut')"
                # so we do NOT touch the big workspace tree at all. Instead install the
                # package alone in a throwaway project (a plain install that succeeds), then
                # copy just that package into the repo's node_modules where stage-native-deps
                # resolves it. Retried: the payload is an HTTPS download that times out on
                # flaky links.
                $gwCode = -1
                $gwVendor = Join-Path $env:TEMP ("hermes-getwin-" + [Guid]::NewGuid().ToString('N').Substring(0, 8))
                # npm writes progress/warnings to stderr; under a caller's EAP=Stop that
                # surfaces as a terminating error before we ever read the exit code.
                # Downgrade for the duration of the call, exactly like the sibling
                # workspace install above, and restore afterwards no matter what.
                $gwPrevEAP = $ErrorActionPreference
                $ErrorActionPreference = "Continue"
                try {
                    New-Item -ItemType Directory -Path $gwVendor -Force | Out-Null
                    '{"name":"hermes-getwin-vendor","version":"1.0.0","private":true}' |
                        Set-Content -LiteralPath (Join-Path $gwVendor 'package.json') -Encoding ASCII
                    $gwVendoredPkg = Join-Path $gwVendor 'node_modules\get-windows'
                    $gwVendoredBinding = Join-Path $gwVendoredPkg 'lib\binding'
                    Push-Location $gwVendor
                    try {
                        for ($try = 1; $try -le 3; $try++) {
                            & $npmExe install get-windows@9.3.0 --include=optional --omit=dev --no-audit --no-fund --no-package-lock 2>&1 |
                                ForEach-Object { "$_" } | Tee-Object -Variable gwOut
                            $gwCode = $LASTEXITCODE
                            if ((Test-Path -LiteralPath $gwVendoredBinding) -and
                    ...
                    if ((Test-Path -LiteralPath $gwVendoredBinding) -and
                        (@(Get-ChildItem -LiteralPath $gwVendoredBinding -Recurse -Filter '*.node' -File -ErrorAction SilentlyContinue).Count -gt 0)) {
                        $gwDest = Join-Path $InstallDir 'node_modules\get-windows'
                        if (Test-Path -LiteralPath $gwDest) {
                            Remove-Item -LiteralPath $gwDest -Recurse -Force -ErrorAction SilentlyContinue
                        }
                        Copy-Item -LiteralPath $gwVendoredPkg -Destination $gwDest -Recurse -Force
                    }
                } finally {
                    Remove-Item -LiteralPath $gwVendor -Recurse -Force -ErrorAction SilentlyContinue
                    $ErrorActionPreference = $gwPrevEAP
                }
                $gwHasBinding = & $gwProbe
                if ($gwHasBinding) {
                    Write-Success "Repaired get-windows native payload"
                } else {
                    throw "get-windows native payload missing after repair (npm exit $gwCode); desktop build cannot stage window enumeration"
                }
            }
        }
    } catch {
    ...
    try {
        ...
        if ($code -ne 0 -and -not $env:ELECTRON_MIRROR -and -not (Test-ElectronDist -InstallDir $InstallDir)) {
            ...
            foreach ($cand in $script:DesktopElectronFallbackMirrors) {
                Write-Warn "Re-downloading Electron via a public mirror ($($cand.Mirror)), then rebuilding:"
                if (-not (Test-ElectronDist -InstallDir $InstallDir)) {
